...
numa única sessão (skill de cada task → implementa → verifica → commit por task com o taskId →
`task_progress`), e chama `EndFeatureRun` uma vez. Fix tasks do orchestrator são steps de UMA task
acima do ship gate. A decomposição em tasks **continua** (boa: é o TODO interno do worker + o
mapa task→assertion da invariante de cobertura), só deixou de ser unidade de spawn. A TUI por-task
sobrevive via eventos `task_completed` (emitidos pelo runner ao completar o impl step) + o tool
`task_progress` (granularidade ao vivo). Preserva budget/resume/orphan/402/inatividade/ship-gate.
