...
| feature→assertion | task→assertion (invariante de cobertura) |
| **1 worker por FEATURE** | **1 worker por feature** (o impl step carrega TODO o plan.json) |
| milestones (cadência) | **colapsa**: 1 feature = 1 ship gate |

### 4.1 Granularidade do worker: 1 por FEATURE, não 1 por task [travado — correção]

**Erro corrigido:** uma versão inicial spawnava **um worker por task** (cada task = uma sessão
`pi --mode rpc` nova). Isso invertia a referência (droid: 1 worker session = 1 feature inteira) e
custava caro: **perda de contexto** entre tasks (sessões distintas), **startup do worker-base
repetido** (≈12 arquivos + init + serviços) N vezes, e **tempo/tokens multiplicados**. A task do
harness era exatamente o que seria um item de TodoWrite *dentro* de uma sessão no droid — promovê-la
a fronteira de processo foi a regressão.

**Modelo atual:** `planFeatureRun` colapsa as tasks num **único impl step** (`IMPL_STEP_ID =
"implement"`) que carrega a lista inteira; UM worker roda o startup 1×, percorre as tasks em ordem
numa única sessão, **puxando cada task com o tool `next_task`** (skill de cada task → implementa →
verifica → commit por task com o taskId), e chama `EndFeatureRun` uma vez. Fix tasks do orchestrator
são steps de UMA task acima do ship gate. A decomposição em tasks **continua** (boa: é o TODO interno
do worker + o mapa task→assertion da invariante de cobertura), só deixou de ser unidade de spawn. A
TUI por-task é DETERMINÍSTICA: o `next_task` grava `task_started`/`task_completed` nas fronteiras (o
harness sequencia, com checagem booleana de git HEAD — sem ler mensagem de commit), com o backstop do
runner ao completar o impl step. Preserva budget/resume/orphan/402/inatividade/ship-gate.
...
├── feature-run.json · status.json · progress_log.jsonl
...
[x] Fatia 6: paridade de resiliência (doc 07) + headless endurecido — workers SESSION-BACKED (`--session-id`), resume real "continue where you left off", kill graceful×hard (status `paused`→re-attach / `running` congelado→orphan requeue via `loadOrBuildFeatureRun`), inactivity watchdog (10 min), auto-pausa 402 (`isUsageLimitEvent`), heartbeat (3 min), retry-budget bônus + preempção no resume. Supervisor fino sobre o pi (NÃO um daemon — ver docs/04-resilience-parity.md). Cobertura: feature-runner/feature-spawn/plan/headless tests.
...
```
[x] Paridade final (gap-scan droid-missions): completion gate determinístico (runLoop só marca
    `completed` com TODAS as assertions `passed` — plan.completionGate; `completion_gate_failed`
    → orchestrator_turn; bypass quando o qa-validator é pulado), session viewer completo no
    cockpit (densidade [ ]/scroll/follow-tail/steer/handoff), Ctrl+R rename no Runs picker +
    per-row load errors, `O` Run Dir, onboarding once-only (ui-state.json) e hints proativos
    de readiness (readiness-hints.ts + cli-hints.json). Ver docs/03-tui.md §Revisão 5.
```
