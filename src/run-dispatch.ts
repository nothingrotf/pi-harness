...
export function buildRunDispatch(featureId: string, tools: DispatchTools = {}): string {
	...
	if (tools.todo) {
		lines.push(
			`${n++}. **Create the Plan with the \`todo\` tool** — one todo per task (in plan.json order), then two trailing todos: "ship gate: harness-code-review" and "ship gate: harness-qa-validator". This Plan is the live source of truth; it survives /reload and compaction — keep it updated at EVERY transition.`,
		);
	}
	...
	lines.push(
		...
		`   b. **harness-qa-validator** — invoke the \`harness-qa-validator\` skill: verify the contract assertions on the real surface and update ${runDir}status.json.`,
