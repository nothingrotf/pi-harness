 ...
 * live tool streaming. The model runs the `harness-setup` skill and writes the profile
 * artifacts under .harness/profile/. The harness stamps profile.json (fingerprint)
 * deterministically afterwards.
 */
import type { DispatchTools } from "./readiness-dispatch.ts";

/** The setup phases (roteiro do dispatch) — numeração 1:1 com o harness-setup SKILL.md. */
export const SETUP_PHASES: readonly string[] = [
	...
	"Phase 5: harness.md (operational overlay) + delivery.json (machine-read branch config)",
...
/** Builds the message that runs the profile setup live. */
export function buildSetupDispatch(_tools: DispatchTools = {}): string {
	const lines = ["Set up the Tier-1 Repo Profile for THIS repository now, live in this session.", ""];
	let n = 1;
	lines.push(`${n++}. Invoke the \`harness-setup\` skill and work through its phases in order:`, ...SETUP_PHASES.map((p) => `   - ${p}`));
	lines.push(
		...
		`${n++}. Author the profile artifacts under .harness/profile/: architecture.md, services.yaml, init.sh, harness.md, delivery.json (the machine-read branch/base/merge config the run-start code parses), skills/<worker-type>/SKILL.md, library/*.md (incl. conventions-map.md + the generic coding-principles.md the ship-gate quality axis scores against). Extract from what the repo already declares first (brownfield); do NOT rewrite the repo's own AGENTS.md/.agents/rules. Verify the commands by executing them before trusting them.`,
