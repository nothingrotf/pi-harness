...
test("buildWorkerBootstrap: ship gate pula worker-base e invoca o validator direto", () => {
	const m = buildWorkerBootstrap(gate, { featureId: "feat-x", workerSessionId: "ws-9" });
	assert.doesNotMatch(m, /worker-base/);
	assert.match(m, /'code-review' skill \(ship-gate validator\)/);
